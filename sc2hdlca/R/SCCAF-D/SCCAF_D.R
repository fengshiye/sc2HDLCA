...
library(reticulate)
####
SCCAF_D=function(bulk=bulk,transformation='none',deconv_type='sc',normalization_C='TMM',normalization_T='TMM',marker_strategy='all',method='DWLS',number_cells=10000,to_remove='none',num_cores=1,NormTrans=TRUE,return_expr=FALSE,batch_key='sampleID',span=0.3,python_home = Sys.which("python")) {
    reticulate::use_python(python_home,required = TRUE)
    source("benchmark1.R")
    ...
    pathway=getwd()
    ####
    reference='/home/ubuntu/data/fengshuo/hdd130/fs/fs/project/devlung/integrated_baron.h5'
    file_name <- basename(reference)  # "integrated_baron.h5"
    reference1 <- tools::file_path_sans_ext(file_name)  # "integrated_baron"
    ####
    ...
    scanpy_workflow(reference,pathway,batch_key=batch_key,span=span)
